...
from ..utils.data.dataset1 import IncrementalPersonReIDSamples
...
class VIPeR(ImageDataset):
    ...
    def __init__(self, datasets_root, relabel=True, combineall=False, split_id=0):
        ...
        self.relabel = relabel
        self.combineall = combineall
        self.cam_a_dir = osp.join(self.root, 'VIPeR', 'cam_a')
        self.cam_b_dir = osp.join(self.root, 'VIPeR', 'cam_b')
        self.split_path = osp.join(self.root, 'splits.json')

        required_files = [self.root, self.cam_a_dir, self.cam_b_dir]
        ...
        super(VIPeR, self).__init__(train, query, gallery)
