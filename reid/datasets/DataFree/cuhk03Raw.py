...
class CUHK03(IncrementalPersonReIDSamples):
    '''
     Dataset statistics:
        - identities: 1360.
        - images: 13164.
    ...
    def __init__(self, datasets_root, relabel=True, combineall=False, split_id=0, cuhk03_labeled=False,
        ...
        self.dataset_dir = osp.join(self.root.replace("_DF", ""))
        ...
        train = [tuple(item + [self.domain_id]) for item in split['train']]
        query = [tuple(item + [self.domain_id]) for item in split['query']]
        gallery = [tuple(item + [self.domain_id]) for item in split['gallery']]
