...
class CUHKSYSU(IncrementalPersonReIDSamples):
    '''
    Duke dataset
    '''
    def __init__(self, datasets_root, relabel=True, combineall=False, use_subset_train=True, **kwargs):
        self.domain_id = kwargs['domain_id']
        self.relabel = relabel
        self.combineall = combineall
        self.root = datasets_root.replace("_DF", "")
        self.use_subset_train = use_subset_train
        # self.dataset_dir = osp.join(self.root, 'cuhk-sysu', 'CUHK-SYSU')
        self.data_path = osp.join(self.root, 'CUHK-SYSU', 'Image', 'SSM')
        self.annotation_path = osp.join(self.root, 'CUHK-SYSU', 'annotation')

        self.processed_dir = osp.join(self.root, 'cuhksysu4reid')
        ...
        if all(map(osp.exists, required_files_state)):
            train = self.process_dir(self.processed_dir_train, relabel=True, camid=0)
            _combine = self.process_dir(self.processed_dir_combine, relabel=True, camid=0)
            query = self.process_dir(self.processed_dir_query, relabel=False, camid=0)
            gallery = self.process_dir(self.processed_dir_gallery, relabel=False, camid=1)
        else:
            ...
            train = self.process_dir(self.processed_dir_train, relabel=True, camid=0)
            _combine = self.process_dir(self.processed_dir_combine, relabel=True, camid=0)
            # https://github.com/KaiyangZhou/deep-person-reid/issues/207
            query = self.process_dir(self.processed_dir_query, relabel=False, camid=0)
            gallery = self.process_dir(self.processed_dir_gallery, relabel=False, camid=1)
    ...
    def process_dir(self, dir_path, relabel=False, **kwargs):
        ...
        for img_path in img_paths:
            ...
            if relabel:
                pid = pid2label[pid]
            data.append((img_path, pid, kwargs['camid'], self.domain_id))
