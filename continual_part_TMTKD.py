...
from reid.visualization import visualization_assignment, visualize_backbone_feat, visualize_tsne
...
from reid.trainer_part import TrainerNCFGTMT
...
def main_worker(args):
    ...
    for idx, domain_name in enumerate(seen_domain):
        ...
        each_domain_class_num.append(seen_domain[domain_name]['domain_class_num'])

        resume_file = osp.join(args.logs_dir, domain_name + "_checkpoint.pth.tar")
        # update old weights file path
        seen_domain[domain_name]['resume_file'] = resume_file
        ...
        if args.evaluate:
            ...
            if training_phase == len(seen_domain):
                final_domain = True
                old_models = old_model_list
            elif training_phase == 1:
                final_domain = False
                old_models = []
            else:
                final_domain = False
                old_models = [old_model_list[-1]]
            if args.resume and os.path.exists(resume_file):
                ...
                if args.header in ["ncforgetpart"]:
                    trainer = TrainerNCFGTMT(
                ...
                for epoch in range(start_epoch, seen_domain[domain_name]['train_epoch']):
                    ...
                    loss_info = trainer.train(
                        ...
                        add_num=old_num_class, old_model_list=old_models,
                        final_domain=final_domain, each_domain_class_num=each_domain_class_num
                    )
                    lr_scheduler.step()
            ...
            if training_phase < len(seen_domain):
                ...
                # Create old frozen model, if the last model, create the model list
                old_model = copy.deepcopy(model)
                old_model = old_model.cuda()
                old_model.eval()
                # update model
                seen_domain[domain_name]['model'] = old_model
                old_model_list.append(old_model)
